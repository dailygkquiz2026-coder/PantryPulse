      ...
      <div className="flex items-center gap-1 p-1 bg-gray-100 dark:bg-cred-gray rounded-2xl overflow-x-auto no-scrollbar max-w-full">
        <button
          ...
          className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shrink-0 flex items-center gap-2 ${
            activeSubTab === 'track' ? 'bg-white dark:bg-cred-dark shadow-sm text-red-600' : 'text-gray-500'
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shrink-0 flex items-center gap-2 ${
            activeSubTab === 'history' ? 'bg-white dark:bg-cred-dark shadow-sm text-red-600' : 'text-gray-500'
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shrink-0 flex items-center gap-2 ${
            activeSubTab === 'analytics' ? 'bg-white dark:bg-cred-dark shadow-sm text-red-600' : 'text-gray-500'
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shrink-0 flex items-center gap-2 ${
            activeSubTab === 'profile' ? 'bg-white dark:bg-cred-dark shadow-sm text-red-600' : 'text-gray-500'
          }`}
        >
          ...
          >
            {!analysisResult ? (
              <div className="space-y-6">
                <div className="cred-card cred-card-glow-red p-8 sm:p-12 text-center space-y-6">
                  <div className="w-20 h-20 bg-red-50 dark:bg-red-950/20 rounded-3xl flex items-center justify-center mx-auto">
                    <Camera className="w-10 h-10 text-red-600" />
                  </div>
                  <div className="space-y-2">
                    <h3 className="text-2xl font-black tracking-tight">Smart Meal Scanner</h3>
                    <p className="text-gray-500 dark:text-gray-400 max-w-xs mx-auto text-sm">
                      Capture or upload a photo of your meal and let AI calculate your intake.
                    </p>
                  </div>

                  <div className="max-w-sm mx-auto space-y-4">
                    <div className="space-y-2 text-left">
                      <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-2">Help AI Identify (Optional)</label>
                      <input 
                        type="text"
                        value={userDescription}
                        onChange={(e) => setUserDescription(e.target.value)}
                        placeholder="e.g. Lentil dal, kadi pakoda, chicken curry"
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 justify-center">
                      <button 
                        ...
                        className="flex-1 px-6 py-4 bg-red-600 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-red-700 transition-all flex items-center justify-center gap-2 shadow-xl shadow-red-200 dark:shadow-none disabled:opacity-50"
                      >
                      ...
                      <button 
                        ...
                        disabled={isAnalyzing}
                        className="flex-1 px-6 py-4 bg-gray-100 dark:bg-cred-gray text-gray-600 dark:text-gray-300 rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-gray-200 dark:hover:bg-cred-dark transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                      >
              ...
              <div className="cred-card cred-card-glow-red p-8 space-y-8">
                <div className="flex items-center justify-between">
                  <h3 className="text-2xl font-black tracking-tight">Analysis Result</h3>
                  <button onClick={() => setAnalysisResult(null)} className="p-2 hover:bg-gray-100 dark:hover:bg-cred-gray rounded-xl transition-all">
                    ...
                    <button
                      ...
                      className={`py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border ${
                        mealType === type 
                          ...
                          : 'bg-gray-50 dark:bg-cred-gray border-gray-100 dark:border-white/5 text-gray-500'
                      }`}
                    >
                      {type}
                    </button>
                ...
                <div className="space-y-4">
                  {analysisResult.map((item, idx) => (
                    <div key={idx} className="p-4 bg-gray-50 dark:bg-cred-gray rounded-2xl border border-gray-100 dark:border-white/5 flex items-center justify-between group">
                      <div className="flex-1">
                        ...
                        <input 
                          ...
                          className="bg-transparent text-xs text-gray-500 font-medium w-full focus:outline-none"
                        />
                      </div>
                      <div className="flex items-center gap-4">
                        <div className="text-right">
                          <input 
                            ...
                            className="bg-transparent font-black text-xl text-red-600 w-20 text-right focus:outline-none"
                          />
                          <p className="text-[8px] font-black uppercase tracking-widest text-gray-400">Calories</p>
                        </div>
                        <button 
                          onClick={() => {
                  ...
                  <button 
                    ...
                    className="w-full py-4 border-2 border-dashed border-gray-200 dark:border-white/5 rounded-2xl text-gray-400 hover:text-red-600 hover:border-red-600 transition-all flex items-center justify-center gap-2 font-bold text-sm"
                  >
                ...
                <div className="pt-6 border-t border-gray-100 dark:border-white/5 flex items-center justify-between">
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">Total Calories</p>
                  ...
                  <button 
                    ...
                    disabled={isSaving}
                    className="px-10 py-4 bg-red-600 text-white rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-red-700 transition-all shadow-xl shadow-red-200 dark:shadow-none disabled:opacity-50"
                  >
          ...
          >
            {logs.length === 0 ? (
              <div className="cred-card p-12 text-center space-y-4">
                ...
                <p className="text-gray-500 font-medium">No meal logs found yet.</p>
              </div>
            ) : (
              logs.map(log => (
                <div key={log.id} className="cred-card p-6 flex items-center justify-between group">
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${
                      log.mealType === 'breakfast' ? 'bg-amber-100 text-amber-600' :
                      log.mealType === 'lunch' ? 'bg-blue-100 text-blue-600' :
                      log.mealType === 'dinner' ? 'bg-purple-100 text-purple-600' :
                      'bg-emerald-100 text-emerald-600'
                    }`}>
                      <Zap className="w-6 h-6" />
                    </div>
                    <div>
                      <h4 className="font-bold text-lg capitalize">{log.mealType}</h4>
                  ...
                  <div className="flex items-center gap-6">
                    <div className="text-right">
                      <p className="text-2xl font-black tracking-tighter">{log.totalCalories}</p>
                      <p className="text-[8px] font-black uppercase tracking-widest text-gray-400">kcal</p>
                    </div>
                    <button 
                      onClick={async () => {
