...
export async function searchCheapestSource(itemName: string, location?: string, previousPurchase?: string, uid?: string) {
  ...
  // Fallback: Gemini without grounding (avoids thinking-model + tools + schema incompatibility)
  if (uid) logAIUsage(uid, 'price_comparison', "gemini-3-flash-preview");
  const ai = getAI();
  const locationContext = location ? ` in India` : ` in India`;
  const purchaseContext = previousPurchase ? ` Previously bought: "${previousPurchase}".` : "";

  const response = await ai.models.generateContent({
    model: "gemini-3-flash-preview",
    contents: `You are a shopping assistant for the Indian grocery market. Estimate current prices for "${itemName}"${locationContext}.${purchaseContext}

Return 4-5 results across Zepto, Blinkit, Swiggy Instamart, BigBasket, and Amazon Fresh.
For each result provide productName, storeName, price (INR number), quantity (pack size), pricePerUnit (number), unit (e.g. "100g"), link (store search URL), sourceVerification ("Estimated price").

Store search URL formats:
- Blinkit: https://blinkit.com/s/?q=[PRODUCT_NAME]
- Zepto: https://www.zepto.com/search?query=[PRODUCT_NAME]
- BigBasket: https://www.bigbasket.com/ps/?q=[PRODUCT_NAME]
- Swiggy Instamart: https://www.swiggy.com/instamart/search?query=[PRODUCT_NAME]
- Amazon Fresh: https://www.amazon.in/s?k=[PRODUCT_NAME]&i=nowstore

Rank by pricePerUnit ascending.`,
    config: {
      responseMimeType: "application/json",
      responseSchema: {
        type: Type.ARRAY,
